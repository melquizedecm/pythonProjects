...
class ListaCircular:
    ...
    def eliminarNodo(self,dato):
        if self.listaVacia():
            ...
            if self.NodoCabeza.getDato()==dato:
                #  1a. caso que el nodoCAbeza sea el unico elemento a eliminar
                if self.NodoCabeza.getRefDerecha() == self.NodoCabeza:
                    self.NodoCabeza=None
                    return True
                else:
                    nodoFinal=self.irAlFinal()
                    self.NodoCabeza=self.NodoCabeza.getRefDerecha()
                    nodoFinal.setRefDerecha(self.NodoCabeza)
                    return True
            else:
    ...
    def buscarNodo(self,dato):
        #####checar el dato a buscar en el NodoCabeza
        nodoGuia=self.NodoCabeza
        while True:
            if nodoGuia.getRefDerecha()!=self.NodoCabeza:
                if nodoGuia.getDato()==dato:
